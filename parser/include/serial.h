...
inline int openSerial(const char* path) {
...
inline bool readLine(int fd, std::string& out, int timeout_ms) {
    out.clear();
    out.reserve(512);
    ...
    while (out.size() < SERIAL_MAX_LINE) {
        ...
        long rem_us = (deadline.tv_sec  - now.tv_sec)  * 1'000'000L
...
inline void drainUntilQuiet(int fd, int quiet_ms) {
    char discard[64];
    while (true) {
...
inline bool sendCommand(int fd, char cmd) {
    if (write(fd, &cmd, 1) != 1) {
        fprintf(stderr, "[serial] write 0x%02X: %s\n", (unsigned char)cmd, strerror(errno));
        return false;
    }
    tcdrain(fd);
    return true;
...
inline bool sendByteWaitEcho(int fd, char c, int timeout_ms = 200) {
    ...
    while (true) {
        ...
        char echo;
        if (read(fd, &echo, 1) > 0) {
            if (echo == c) return true;
            if (echo == '?') {
                fprintf(stderr, "[serial] controller rejected byte 0x%02X\n", (unsigned char)c);
                return false;
            }
        }
    }
...
// e.g. "&GAA0500", "&H1C01"
inline bool sendAmpersandCommand(int fd, const char* cmd) {
    for (int i = 0; cmd[i] != '\0'; ++i) {
        if (!sendByteWaitEcho(fd, cmd[i])) {
