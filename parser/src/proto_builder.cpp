...
mppt::DataloggerPayload build_datalogger_summary_proto(const DataloggerSummary &summary,
                                                       std::time_t              ts) {
...
mppt::DataloggerPayload build_datalogger_monthly_proto(const MonthlyLogBuffer &months,
                                                       std::time_t             ts) {
    mppt::DataloggerPayload msg;
    msg.set_timestamp(static_cast<uint32_t>(ts));
...
mppt::DeviceInfo
build_device_info_proto(const EepromSettings &cfg, uint32_t fw_version, std::time_t ts) {
    mppt::DeviceInfo msg;
