#include "space_parser.h"
#include "eeprom_parser.h"
#include "printer.h"
#include "json_builder.h"
#include "utils.h"

#include <cstdio>
#include <fstream>
#include <string>
#include <vector>


int main(int argc, char* argv[]) {
    if (argc < 2) {
        fprintf(stderr, "Usage: %s <logfile.txt>\n", argv[0]);
        return 1;
    }

    std::ifstream file(argv[1]);
    if (!file.is_open()) {
        fprintf(stderr, "Error: cannot open '%s'\n", argv[1]);
        return 1;
    }

    PhocosHeader            hdr{};
    PhocosTelemetry         tele{};
    EepromConfig            cfg{};
    DataloggerSummary       summary{};
    std::vector<DailyLog>   daily_logs;
    std::vector<MonthlyLog> monthly_logs;

    bool have_tele   = false;
    bool have_eeprom = false;

    int lines_total   = 0;
    int lines_skipped = 0;
    int records_ok    = 0;
    int records_fail  = 0;

    std::string line;

    while (std::getline(file, line)) {
        ++lines_total;

        if (!line.empty() && line.back() == '\r') {
            line.pop_back();
        }

        // TODO remove in prod as this is used for local log parsing
        // Banner header lines (start with '*')
        if (!line.empty() && line[0] == '*') {
            parseHeaderLine(line, hdr);
            ++lines_skipped;
            continue;
        }

        // EEPROM dump line (starts with '!')
        if (isEepromLine(line)) {
            std::string dump = line.substr(1);
            if (parseEepromDump(dump.c_str(), dump.size(), cfg, summary, daily_logs, monthly_logs)) {
                have_eeprom = true;
            }
            ++lines_skipped;
            continue;
        }

        // Settings line (starts with '"')
        // TODO wire with the parser
        if (isSettingsLine(line)) {
            ++lines_skipped;
            continue;
        }

        // Skip anything that doesn't look like a Space line
        if (!isSpaceLine(line)) {
            ++lines_skipped;
            continue;
        }

        // Space Command response
        tele = PhocosTelemetry{};
        if (parsePhocosLine(line.c_str(), line.size(), tele)) {
            ++records_ok;
            have_tele = true;
        }
        else {
            ++records_fail;
            fprintf(stderr, "PARSE FAILED (line %d, len=%zu): %.100s...\n",
                    lines_total, line.size(), line.c_str());
        }
    }

    if (!have_tele && !have_eeprom) {
        fprintf(stderr, "No parseable data found in '%s'\n", argv[1]);
        return 1;
    }

    char ts[16];
    currentTimestamp(ts, sizeof(ts));

    // hw_version priority: EEPROM byte 120 > field count > banner
    // TODO remove banner checks in prod, used for local log parsing only
    if (!have_eeprom || cfg.hw_version == 0) {
        cfg.hw_version = have_tele ? tele.hw_version : hdr.hw_version;
    }

    if (have_tele)   { printSystemState(hdr, tele, cfg, ts); }
    if (have_eeprom) { printEepromConfig(cfg); }
    if (have_eeprom) { printDataLogger(summary, daily_logs, monthly_logs); }

    nlohmann::json j = buildJSON(hdr, tele, cfg, summary, daily_logs, monthly_logs, ts);

    const std::string& serial = !cfg.serial_number.empty() ? cfg.serial_number
                               : !hdr.serial_number.empty() ? hdr.serial_number : "";
    publishJSON(j, serial.c_str());

    printf("\n────────────────────────────────────────────\n");
    printf("  Lines read   : %d\n", lines_total);
    printf("  Lines skipped: %d  (header / EEPROM / settings)\n", lines_skipped);
    printf("  Records OK   : %d\n", records_ok);
    printf("  Records FAIL : %d\n", records_fail);
    printf("────────────────────────────────────────────\n");

    return (records_fail == 0) ? EXIT_SUCCESS : EXIT_FAILURE;
}
