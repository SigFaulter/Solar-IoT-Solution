...
mppt_ChargeMode charge_mode_from_flags(const ChargeStatusFlags &f) {
  if (f.boost_charge)
    return mppt_ChargeMode_CHARGE_MODE_BOOST;
  if (f.equalization_charge)
    return mppt_ChargeMode_CHARGE_MODE_EQUALIZATION;
...
mppt_DeviceSettings build_device_settings(const EepromData &e,
  ...
  msg.has_battery_type = true;
  {
    ...
    msg.battery_type =
        static_cast<mppt_BatteryType>(e.battery_type_idx + (is_v2 ? 1 : 4));
  }
  ...
  msg.advanced_flags = (e.dali_enable ? 1u : 0u) | (e.alc_enable ? 2u : 0u);
