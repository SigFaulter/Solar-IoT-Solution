...
// Cached hardware version - set once from EEPROM, never changes for a given device
...
static void publish_fault_status(bool force = false) {
    if (!g_tele_ok || g_hw_version != 3) return;
...
static void handle_command(const uint8_t *data, size_t len) {
    ...
    if (!pb_decode(&stream, mppt_ControlCommand_fields, &cmd)) return;
...
class MyCharacteristicCallbacks : public NimBLECharacteristicCallbacks {
    ...
    void onSubscribe(NimBLECharacteristic *ch, NimBLEConnInfo &, uint16_t subValue) override {
...
static bool repoll_eeprom() {
    // TODO: send '!' over UART2, read response into g_eeprom; return true on change
    return false;
}
