...
bool parse_space_line(const char *line, SpaceTelemetry &out) {
  ...
  while (*p) {
    ...
    case FIELD_CHARGE_CURRENT_MA:
      out.charge_current_ma10 = val;
      break;
    case FIELD_LOAD_CURRENT_MA:
      out.load_current_ma10 = val;
      break;
    case FIELD_PV_VOLTAGE_MV:
      out.pv_voltage_mv = val;
      break;
    case FIELD_PV_TARGET_MV:
      out.pv_target_mv = val;
      break;
    case FIELD_PWM_COUNTS:
      out.pwm_counts = (uint16_t)val;
      break;
    ...
    case FIELD_BATTERY_VOLTAGE_MV:
      out.battery_voltage_mv = val;
      break;
    case FIELD_BAT_THRESHOLD_MV:
      out.bat_threshold_mv = val;
      break;
    case FIELD_BATTERY_SOC_PCT:
      out.battery_soc_pct = (uint8_t)val;
      break;
    case FIELD_INTERNAL_TEMP_C:
      out.internal_temp_c = (int16_t)val;
      break;
    case FIELD_EXTERNAL_TEMP_C:
      out.external_temp_c = (int16_t)val;
      break;
    ...
    case FIELD_LED_CURRENT_MA:
      out.led_current_ma10 = val;
      break;
    case FIELD_LED_STATUS:
      out.led_status = (uint8_t)val;
      break;
    case FIELD_DALI_ACTIVE:
      out.dali_active = (uint8_t)val;
      break;
    case FIELD_OP_DAYS:
      out.op_days = (uint16_t)val;
      break;
    case FIELD_BAT_OP_DAYS:
      out.bat_op_days = (uint16_t)val;
      break;
    case FIELD_ENERGY_IN_WH:
      out.energy_in_wh = (uint16_t)val;
      break;
    case FIELD_ENERGY_OUT_WH:
      out.energy_out_wh = (uint16_t)val;
      break;
    case FIELD_ENERGY_RETAINED_WH:
      out.energy_retained_wh = (uint16_t)val;
      break;
    case FIELD_CHARGE_POWER_W:
      out.charge_power_w = (uint16_t)val;
      break;
    case FIELD_LOAD_POWER_W:
      out.load_power_w = (uint16_t)val;
      break;
    case FIELD_LED_POWER_W:
      out.led_power_w = (uint16_t)val;
      break;
    ...
    case FIELD_PV_DETECTED:
      if (out.hw_version == 3)
        out.pv_detected = (uint8_t)val;
      break;
    case FIELD_BATTERY_DETECTED:
      if (out.hw_version == 3)
        out.battery_detected = (uint8_t)val;
      break;
    }
...
bool parse_eeprom_line(const char *line, EepromData &out) {
  ...
  while (*p && byte_count < (int)sizeof(bytes)) {
    ...
    if (*p && *p != ';' && *p != '\r' && *p != '\n')
      hex[1] = *p++;
    bytes[byte_count++] = static_cast<uint8_t>(strtol(hex, nullptr, 16));
  }
  ...
  // Datalogger summary
  if (byte_count > (EEPROM_DATALOG_SUMMARY_OFFSET + 16)) {
    ...
    out.avg_morning_soc_pct =
        (out.num_days > 0) ? (morning_soc_sum * 6.6f / out.num_days) : 0.0f;
  }

  // Daily log entries
  out.daily_count = 0;
  int num_daily = std::min((int)out.num_days, (int)EEPROM_DAILY_MAX_BLOCKS);
  int start_daily = (out.num_days > EEPROM_DAILY_MAX_BLOCKS)
                        ? (out.num_days % EEPROM_DAILY_MAX_BLOCKS)
                        : 0;
  ...
  for (int i = 0; i < num_daily; i++) {
    ...
    if (off + EEPROM_DAILY_BLOCK_SIZE > byte_count)
      break;

    // Skip blank blocks
    if (b[off] == 0 && b[off + 1] == 0 && u16be(b, off + 2) == 0 &&
        u16be(b, off + 4) == 0)
      continue;
  ...
  int start_monthly = (total_months > EEPROM_MONTHLY_MAX_BLOCKS)
                          ? (total_months % EEPROM_MONTHLY_MAX_BLOCKS)
                          : 0;
  ...
  for (int i = 0; i < num_monthly; i++) {
    ...
    if (off + EEPROM_MONTHLY_BLOCK_SIZE > byte_count)
      break;
  ...
  printf("[DEBUG] Datalogger: %d days, %d daily logs, %d monthly logs, "
         ...
         out.total_ah_charge_mah / 1000.0);
