...
bool parse_space_line(const char *line, SpaceTelemetry *out) {
    ...
    bool success = field_idx >= expected && out->battery_voltage_mv != 0;
    if (success) {
        printf("Parsed Telemetry: Vbat=%lumV, Isolar=%lumA, Vpv=%lumV, SOC=%u%%, Temp=%dC, Faults=0x%02x\n",
               out->battery_voltage_mv, out->charge_current_ma10 * 10, out->pv_voltage_mv,
               out->battery_soc_pct, out->internal_temp_c, out->fault_status);
    } else {
        printf("Telemetry parse failed: fields=%d/%d, Vbat=%lumV\n", field_idx, expected, out->battery_voltage_mv);
    }
