...
void mppt_ble_send(mppt_msg_type_t msg_type, const uint8_t *payload,
                   uint16_t len) {
  uint16_t conn = ble_gatt_conn_handle();
  if (conn == BLE_HS_CONN_HANDLE_NONE || !ble_gatt_subscribed())
    return;
  ...
  while (offset < total) {
    uint16_t take =
        ((total - offset) < chunk_max) ? (total - offset) : chunk_max;
...
bool mppt_reassembler_feed(mppt_reassembler_t *r, const uint8_t *data,
                           uint16_t len) {
  if (len == 0)
    return false;
