...
bool encode_log_entries(pb_ostream_t *stream, const pb_field_t *field,
                        void *const *arg) {
  ...
  for (uint8_t i = 0; i < l->count; i++) {
    ...
    le.soc_pct = (uint32_t)e->soc_pct;
    le.ext_temp_max_c = e->ext_temp_max_c;
    le.ext_temp_min_c = e->ext_temp_min_c;
    le.nightlength_min = e->nightlength_min;
    le.state_flags = state_flags_to_bitmask(&e->state_flags);
    if (!pb_encode_tag_for_field(stream, field))
      return false;
    if (!pb_encode_submessage(stream, mppt_LogEntry_fields, &le))
      return false;
  }
  return true;
}

static mppt_ChargeMode charge_mode_from_state(uint16_t state) {
  if (state & 0x20) return mppt_ChargeMode_CHARGE_MODE_DISABLED;
  if (state & 0x01) return mppt_ChargeMode_CHARGE_MODE_BOOST;
  if (state & 0x02) return mppt_ChargeMode_CHARGE_MODE_EQUALIZATION;
  return mppt_ChargeMode_CHARGE_MODE_FLOAT;
}

uint32_t make_tele_flags(const SpaceTelemetry *t) {
  uint32_t f = 0;
  if (t->battery_detected)
    f |= (1u << 0);
  if (t->charge_flags.is_night)
    f |= (1u << 1);
  if (load_status_is_on(&t->load_flags))
    f |= (1u << 2);
  if (t->load_flags.night_mode_active)
    f |= (1u << 3);
  if (t->load_flags.lvd_active)
    f |= (1u << 4);
  if (t->load_flags.user_disconnect)
    f |= (1u << 5);
...
bool write_string_callback(pb_ostream_t *stream, const pb_field_t *field,
                           void *const *arg) {
...
mppt_Telemetry build_telemetry(const SpaceTelemetry *t, uint32_t ts) {
  ...
  msg.charge_mode = charge_mode_from_state(t->charge_state_raw);
  ...
  msg.led_status = (mppt_LedStatus)t->led_status;
...
mppt_FaultStatus build_fault_status(const SpaceTelemetry *t, uint32_t ts) {
  ...
  msg.timestamp = ts;
  msg.fault_mask = fault_status_to_bitmask(&t->fault_flags);
  msg.load_state_mask = t->load_state_raw;
  msg.charge_state_mask = t->charge_state_raw;
  return msg;
}

mppt_DeviceInfo build_device_info(const SpaceTelemetry *t, const EepromData *e,
                                  uint32_t ts) {
  ...
  msg.temp_comp_mv_per_c = e->temp_comp;

  static char date_buf[16];
  snprintf(date_buf, sizeof(date_buf), "20%02d-%02d-%02d",
           bcd_to_dec(e->prod_year_hi), bcd_to_dec(e->prod_month),
           bcd_to_dec(e->prod_day));
  ...
  msg.device_type.arg = (void *)((e->device_identifier == 0x52) ? "Solar Smart Controller V2" : "Solar Smart Controller V3");

  return msg;
}

mppt_DeviceSettings build_device_settings(const EepromData *e,
  ...
  {
    const bool is_v2 = (e->device_identifier == 0x52);
    // V2: AGM=0, Liquid=1, LFP=2 -> Proto: 1, 2, 3
    // V3: LFP Hi=0, Med=1, Lo=2 -> Proto: 4, 5, 6
    msg.battery_type = (mppt_BatteryType)(e->battery_type_idx + (is_v2 ? 1 : 4));
  }
  ...
  msg.lvd_mode = e->lvd_mode_voltage ? mppt_LvdMode_LVD_MODE_VOLTAGE
                                    : mppt_LvdMode_LVD_MODE_SOC;
  msg.has_night_mode = true;
  msg.night_mode = (mppt_NightMode)e->night_mode;
  ...
  msg.dimming_mode = (mppt_NightMode)e->dim_mode;
  ...
  msg.advanced_flags = (uint32_t)((e->dali_enable ? 1u : 0u) | (e->alc_enable ? 2u : 0u));

  if (override) {
    if (override->has_battery_type)
      msg.battery_type = override->battery_type;
    if (override->has_capacity_ah)
      msg.capacity_ah = override->capacity_ah;
    if (override->has_lvd_voltage_mv)
      msg.lvd_voltage_mv = override->lvd_voltage_mv;
    if (override->has_lvd_mode)
      msg.lvd_mode = override->lvd_mode;
    if (override->has_night_mode)
      msg.night_mode = override->night_mode;
    if (override->has_evening_minutes)
      msg.evening_minutes = override->evening_minutes;
    if (override->has_morning_minutes)
      msg.morning_minutes = override->morning_minutes;
    if (override->has_night_threshold_mv)
      msg.night_threshold_mv = override->night_threshold_mv;
    if (override->has_dimming_mode)
      msg.dimming_mode = override->dimming_mode;
    if (override->has_evening_minutes_dimming)
      msg.evening_minutes_dimming = override->evening_minutes_dimming;
    if (override->has_morning_minutes_dimming)
      msg.morning_minutes_dimming = override->morning_minutes_dimming;
    if (override->has_dimming_pct)
      msg.dimming_pct = override->dimming_pct;
    if (override->has_base_dimming_pct)
      msg.base_dimming_pct = override->base_dimming_pct;
    if (override->has_advanced_flags)
      msg.advanced_flags = override->advanced_flags;
  }
