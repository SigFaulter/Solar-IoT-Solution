...
static bool apply_commands_to_mppt(char cmds[][16], int count) {
    for (int i = 0; i < count; i++) {
        ESP_LOGI(TAG, "Sending MPPT command: %s", cmds[i]);
        // TODO: Implement UART write and wait for echo
...
void publish_device_settings(void) {
    ...
    mppt_DeviceSettings msg = build_device_settings(s_eeprom, NULL, now_sec());
...
void handle_command(const uint8_t *data, uint16_t len) {
    ...
    case mppt_ControlCommand_set_settings_tag: {
        char cmds[32][16];
        int count = 0;
        build_write_commands(&cmd.payload.set_settings, *s_hw_version, cmds, &count);
        ok = apply_commands_to_mppt(cmds, count);
        reason = ok ? "settings applied to mppt" : "mppt write failed";
        break;
    }
    case mppt_ControlCommand_switch_load_tag: {
        bool on = (cmd.payload.switch_load.flags & 1u) != 0;
        char k_cmd[1][16];
        snprintf(k_cmd[0], 16, "&K55%02X00", on ? 1 : 0);
        ok = apply_commands_to_mppt(k_cmd, 1);
        reason = ok ? (on ? "load on" : "load off") : "mppt write failed";
        break;
    }
    }
    ...
    if (ok && cmd.which_payload == mppt_ControlCommand_set_settings_tag) {
        ...
        // In real app, we would re-poll EEPROM here. For now just publish what we know.
